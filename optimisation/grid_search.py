    ...
    def optimise(self, exogenous_data):
        ...
        for (i, k_on) in enumerate(np.logspace(-2, 2, num=on_count)):
            for (j, k_off) in enumerate(np.logspace(-2, 2, num=off_count)):
                ...
                print(
                    f"{'%.2f' % (100 * (i * (off_count + 1) + j + 1) / (i * on_count + off_count))}%"
                )

        print(f"{time.time() - start}s elapsed")

        np.save(f"cache/res_real_tf{tf_index}_{on_count}_{off_count}", res_real)
        np.save(f"cache/res_prob_tf{tf_index}_{on_count}_{off_count}", res_prob)

        fig, axes = plt.subplots(1, 2, sharey=True)
        for ax, res in zip(axes, (res_real, res_prob)):
            ax.imshow(res, cmap="cvidis", interpolation="none", vmin=0, vmax=2)
        fig.colorbar()
        plt.show()
